...
async function fetchTotalCount() {
  try {
    const res = await fetch('/api/consumer/count')
    if (res.ok) {
...
onMounted(async () => {
  fetchTotalCount()
  try {
    const res = await fetch('/api/consumer/status')
    if (res.ok) {
      const data = await res.json()
...
async function toggleConsumer() {
  ...
  try {
    if (action === 'start') {
      ...
      await connectWebSocket()
      const res = await fetch('/api/consumer/start', { method: 'POST' })
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
      running.value = true
    } else {
      const res = await fetch('/api/consumer/stop', { method: 'POST' })
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
      running.value = false
      disconnectWebSocket()
    }
